...
export async function confirmProposal(userId, id) {
  ...
  // E5: 提案採用時にメモリーを自動追加（append-only, learning loop）
  try {
    await prisma.memory.create({
      data: {
        userId,
        text: `[採用] ${proposal.title}: ${proposal.body}`,
        sourceRefs: JSON.stringify([{ type: 'proposal', id: proposal.id }]),
      },
    })
  } catch (memoryError) {
    // メモリー追加失敗は提案確定に影響させない（best-effort）
    logger.warn('Failed to auto-create memory on proposal confirm', {
