...
import { z } from 'zod'
import { TASK_STATUS, STT_STATUS, PROPOSAL_STATUS, NODE_TYPE, RULE_TREE_MAX_DEPTH } from './constants.js'
 ...
 * PENDING      |    -    |    OK      |  OK  |  OK
 * PROCESSING   |   NG    |    -       |  OK  |  OK
 * DONE         |   NG    |    NG      |  -   |  NG  (terminal)
 * FAILED       |   NG    |    OK      |  NG  |  -   (retry)
 *
 * PENDING→DONE: 許可 (サーバーサイド同期STTでPROCESSINGをスキップ)
 * PENDING→PROCESSING: 許可 (Flutter非同期STTフロー)
 */
export const STT_ALLOWED_TRANSITIONS = Object.freeze({
  [STT_STATUS.PENDING]: [STT_STATUS.PROCESSING, STT_STATUS.DONE, STT_STATUS.FAILED],
  [STT_STATUS.PROCESSING]: [STT_STATUS.DONE, STT_STATUS.FAILED],
  [STT_STATUS.DONE]: [], // 最終状態、遷移不可
  [STT_STATUS.FAILED]: [STT_STATUS.PROCESSING], // リトライのみ許可
})

/**
 * STT ステータス遷移を検証
 * @param {string} fromStatus - 現在の STT ステータス
 * @param {string} toStatus   - 遷移先の STT ステータス
 * @returns {{ valid: boolean, message?: string }}
 */
export function validateSttTransition(fromStatus, toStatus) {
  // 同一ステータスは冪等操作として許可（リトライ対応）
  if (fromStatus === toStatus) {
    return { valid: true }
  }

  const allowed = STT_ALLOWED_TRANSITIONS[fromStatus]
  if (!allowed) {
    return { valid: false, message: `Unknown STT status: ${fromStatus}` }
  }

  if (!allowed.includes(toStatus)) {
    return { valid: false, message: `STT transition from ${fromStatus} to ${toStatus} is not allowed` }
  }
 ...
 * REJECTED  → [] (最終状態)
 */
const PROPOSAL_ALLOWED_TRANSITIONS = Object.freeze({
  [PROPOSAL_STATUS.PENDING]: [PROPOSAL_STATUS.CONFIRMED, PROPOSAL_STATUS.REJECTED],
  [PROPOSAL_STATUS.CONFIRMED]: [], // 最終状態、復帰不可
  [PROPOSAL_STATUS.REJECTED]: [],  // 最終状態、復帰不可
})
 ...
 * @returns {{ valid: boolean, message?: string }}
 */
export function validateProposalTransition(fromStatus, toStatus) {
  if (fromStatus === toStatus) {
    return { valid: false, message: `Already in status: ${fromStatus}` }
  }

  const allowed = PROPOSAL_ALLOWED_TRANSITIONS[fromStatus]
  if (!allowed) {
    return { valid: false, message: `Unknown proposal status: ${fromStatus}` }
  }

  if (!allowed.includes(toStatus)) {
    return {
      valid: false,
      message: `Transition from ${fromStatus} to ${toStatus} is not allowed. Allowed: [${allowed.join(', ') || 'none'}]`,
    }
  }
...
export const ruleTreeUpdateSchema = z.object({
  nodes: z.array(
    z.object({
      id: z.string().min(1),
      parentId: z.string().nullable().optional(),
      type: z.string().min(1),
      bunjinSlug: z.string().nullable().optional(),
    })
  ).min(1, 'nodes must contain at least one element'),
})
