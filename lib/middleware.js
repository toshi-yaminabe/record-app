...
export function withApi(handler, options = {}) {
  ...
  return async (request, routeContext) => {
    try {
      ...
      // 2. 認証
      if (cronMode) {
        // Cronモード: CRON_SECRET認証（タイミング攻撃耐性あり）
...
async function authenticateUser(request) {
  ...
  // Supabase Auth 設定チェック
  const configStatus = getSupabaseAuthConfigStatus()

  // 開発環境でSupabaseが未設定の場合はモックユーザーにフォールバック
  if (!configStatus.ok && process.env.NODE_ENV !== 'production') {
    return MOCK_USER_ID
  }
