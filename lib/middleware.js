...
const MOCK_DEV_USER_ID = 'mock-user-001'
...
export function withApi(handler, options = {}) {
  ...
  return async (request, routeContext) => {
    try {
      ...
      const rateLimitResult = await checkRateLimit(rateLimitId, rateLimit)
...
async function authenticateUser(request) {
  // 認証バイパス（Supabase Auth導入までの暫定措置）
  if (process.env.DEV_AUTH_BYPASS === 'true') {
    return MOCK_DEV_USER_ID
  }
